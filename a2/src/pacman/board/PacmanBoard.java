...
public class PacmanBoard {
    ...
    /**
     * Checks if another object instance is equal to this instance. Boards are
     * equal if they have the same dimensions (width, height) and have equal
     * items for all board positions.
     * @param o board to compare.
     * @return true if same, false otherwise.
     */
    @Override
    public boolean equals(Object o) {
        try {
            PacmanBoard newBoard = (PacmanBoard) o;
            if (this.getHeight() != newBoard.getHeight() ||
                    this.getWidth() != newBoard.getWidth()) {
                return false;
            } else {
                for (int i = 0; i < this.getHeight(); i++) {
                    for (int j = 0; j < this.getWidth(); j++) {
                        BoardItem b1 = this.getEntry(new Position(i, j));
                        BoardItem b2 = newBoard.getEntry(new Position(i, j));
                        if (!b1.equals(b2)) {
                            return false;
                        }
                    }
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < this.getHeight(); i++) {
            for (int j = 0; j < this.getWidth(); j++) {
                sumScore =+ this.getEntry(new Position(i, j)).getScore();
            }
        }
        sumScore = sumScore + (this.getWidth() * this.getHeight());
        return sumScore;
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.getHeight(); i++) {
            for (int j = 0; j < this.getWidth(); j++) {
                String newString = String.valueOf(this.getEntry(new Position(i,
                        j)).getChar());
                board = board + newString;
            }
            if (i < this.height - 1) {
